...
const AssignComplaints = () => {
    ...
    const handleAssignStaff = async (staff) => {
        try {
            const res = await axios.patch(`${API_URL}/${selectedComplaint._id}/assign`, {
                staffId: staff._id,
            });
        ...
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            <main className="max-w-[1600px] mx-auto p-6">
                <div className="mb-6 relative max-w-md">
                    ...
                    <input type="text" placeholder="Search student..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" onChange={(e) => setSearchTerm(e.target.value)} />
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    <table className="w-full text-center border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100 text-sm">
                            {complaints.filter((c) => c.student?.name?.toLowerCase().includes(searchTerm.toLowerCase())).length > 0 ? (
                                complaints
                                    ...
                                    .map((item) => (
                                        <tr key={item._id} className="hover:bg-slate-50/50">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-bold border ${getStatusStyles(item.status)}`}>{item.status || "Pending"}</span>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-bold transition-all border ${item.assignedTo ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-indigo-50 text-indigo-600 border-indigo-100 hover:bg-indigo-600 hover:text-white"}`}
                                                >
                                                    <UserCheck size={14} />
                                ...
                                <tr>
                                    <td colSpan="5" className="px-6 py-6 text-center text-slate-400 italic">
                                        No complaints found matching
                                    </td>
                                </tr>
                            )}
                        </tbody>
                    ...
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
                        ...
                        <div className="p-4 max-h-[400px] overflow-y-auto">
                            ...
                            <div className="space-y-1">
                                {staffList.map((staff) => (
                                    <button key={staff._id} onClick={() => handleAssignStaff(staff)} className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-indigo-50 text-left group transition-all border border-transparent hover:border-indigo-100">
                        ...
                        <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-end">
                            <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700">
                                Cancel
                            </button>
                        </div>
                    </div>
